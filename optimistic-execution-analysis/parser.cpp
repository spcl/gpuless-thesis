...
std::vector<AlignedParameter> parse_ptr_parameters(std::ifstream& input, const std::string& kernel_mangle) {
    ...
    while (getline(input, line)) {
        ...
        if(splitted_line[1] == ".align") {
            ...
            uint64_t buffer_size = std::stoi(splitted_buffer[1].substr(0, splitted_buffer[1].size() - 1));
    ...
    for(const auto& buffer : buffers) {
        if(buffer.second.size == 0) {
            ...
            for(uint64_t offset = 0; offset < buffer.second.size; offset += step) {
    ...
    for (const auto& param : is_ptr) {
        for(const auto& offset : param.second) {
            if(offset.second) {
                pointer_parameters.push_back({param.first, offset.first, buffers[param.first].alignment});
            }
        }
    }
