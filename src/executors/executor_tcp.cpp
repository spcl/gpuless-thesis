...
bool executor_tcp::negotiate_session(
...
bool executor_tcp::query_device_attributes() {
    ...
    auto attributes_request_msg =
        CreateProtocolMessage(builder, Message_AttributesRequest,
    ...
    auto attributes =
        execution_answer_msg->message_as_AttributesAnswer()->attributes();
...
bool executor_tcp::init(const char *ip, const short port,
                        gpuless::manager::instance_profile profile) {
...
bool executor_tcp::get_device_attribute(int *value,
                                        CUdevice_attribute attribute) {
    *value = this->device_attributes[static_cast<int32_t>(attribute)];
    return true;
}

bool executor_tcp::execute(const char *kernel, dim3 dim_grid, dim3 dim_block,
    ...
    for (const auto &a : args) {
        auto fba =
            CreateKernelArgument(builder, builder.CreateString(a.id), a.flags,
                                 builder.CreateVector(a.buffer));
        fb_args.push_back(fba);
    }
    auto execution_request_msg = CreateProtocolMessage(
        builder, Message_ExecutionRequest,
        CreateExecutionRequest(builder, builder.CreateString(kernel),
                               builder.CreateVector(this->cuda_bin),
                               builder.CreateVector(fb_args), &fb_dim_grid,
                               &fb_dim_block)
            .Union());
    ...
    auto return_buffers =
        execution_answer_msg->message_as_ExecutionAnswer()->return_buffers();
