...
for i in range(3, 29):
    ...
    lines = [x.rstrip() for x in lines]
    for l in lines:
        data = data.append(pd.DataFrame([[100, 'no network', 2**i * 4, float(l)]], columns=cols),
                           ignore_index=True)
...
ax.text(2**20 + 2**24, 0.6, '1 MB', transform=trs, color='green')
ax.axvline(x=2**20*256, color='green', linestyle='--') # 256 MB
ax.text(2**20*256 + 2**24, 0.6, '256 MB', transform=trs, color='green')
ax.axvline(x=2**30, color='green', linestyle='--') # 1 GB
ax.text(2**30 + 2**24, 0.6, '1 GB', transform=trs, color='green')
